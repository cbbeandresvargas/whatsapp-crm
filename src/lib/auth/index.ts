...
const globalForSignup = globalThis as unknown as {
  __crmInternalSignup?: AsyncLocalStorage<boolean>;
};
...
function internalSignupContext(): AsyncLocalStorage<boolean> {
  if (!globalForSignup.__crmInternalSignup) {
    globalForSignup.__crmInternalSignup = new AsyncLocalStorage<boolean>();
  }
  return globalForSignup.__crmInternalSignup;
}
